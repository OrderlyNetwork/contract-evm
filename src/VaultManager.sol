...
contract VaultManager is Ownable {
    ...
    // get balance
    function getBalance(uint256 _chainId, bytes32 _symbol) public view returns (uint256) {
        return chain2symbol2balance[_chainId][_symbol];
    }

    // add balance
    function addBalance(uint256 _chainId, bytes32 _symbol, uint256 _deltaBalance) public onlyLedger {
        chain2symbol2balance[_chainId][_symbol] += _deltaBalance;
    }

    // sub balance
    function subBalance(uint256 _chainId, bytes32 _symbol, uint256 _deltaBalance) public onlyLedger {
        chain2symbol2balance[_chainId][_symbol] -= _deltaBalance;
    }
}