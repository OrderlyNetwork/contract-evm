...
abstract contract DecompressorExtension {
     ...
     */
    function decompress(bytes calldata data) external payable {
        _delegatecall(decompressed(data));
    }
     ...
     */
    function decompressed(bytes calldata data) public view returns (bytes memory raw) {
        return _decompressed(data);
    }
